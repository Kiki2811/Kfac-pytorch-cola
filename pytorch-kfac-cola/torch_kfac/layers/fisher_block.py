...
class FisherBlockOperator(LinearOperator):
    def __init__(self, activation_cov: torch.Tensor, sensitivity_cov: torch.Tensor, renorm_coeff: float):
        self.activation_cov = activation_cov
        self.sensitivity_cov = sensitivity_cov
        self.renorm_coeff = renorm_coeff

    def matvec(self, x):
        # Implement the matrix-vector multiplication using the Fisher block
        return self.sensitivity_cov @ x @ self.activation_cov / self.renorm_coeff
...
class FisherBlock(object):
    ...
    def multiply(self, grads: Iterable[torch.Tensor], damping: torch.Tensor) -> Iterable[torch.Tensor]:
        ...
        # Create the CoLA operator with damping applied
        operator = FisherBlockOperator(act_cov + torch.eye(act_cov.shape[0], device=a_damp.device) * a_damp,
                                      sen_cov + torch.eye(sen_cov.shape[0], device=s_damp.device) * s_damp,
                                      self.renorm_coeff)
        
        # Apply CoLA's matvec function for the matrix-vector multiplication
        mat_grads = self.grads_to_mat(grads)
        nat_grads = operator.matvec(mat_grads)
    ...
    def multiply_preconditioner(self, grads: Iterable[torch.Tensor], damping: torch.Tensor) -> Iterable[torch.Tensor]:
        ...
        # Create inverse operators for CoLA
        act_cov_inverse = inverse_by_cholesky(act_cov, a_damp)
        sen_cov_inverse = inverse_by_cholesky(sen_cov, s_damp)
        
        # Create the CoLA operator for preconditioner
        operator = FisherBlockOperator(act_cov_inverse, sen_cov_inverse, self.renorm_coeff)
    ...
    def get_fisher_components(self) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        if hasattr(self, '_activations_cov') and hasattr(self, '_sensitivities_cov'):
            a_cov = self._activations_cov._var
            s_cov = self._sensitivities_cov._var
            return a_cov, s_cov
        return None, None
